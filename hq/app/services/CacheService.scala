...
class CacheService(
  ...
  if (environment.mode != Mode.Test) {
    ...
    val publicBucketsSubscription = Observable.interval(initialDelay + 5.minutes, 5.minutes).subscribe { _ =>
      refreshPublicBucketsBox()
    }

    val exposedKeysSubscription = Observable.interval(initialDelay + 10.minutes, 5.minutes).subscribe { _ =>
      refreshExposedKeysBox()
    }

    val sgSubscription = Observable.interval(initialDelay + 15.minutes, 5.minutes).subscribe { _ =>
      refreshSgsBox()
    }

    val credentialsSubscription = Observable.interval(initialDelay + Duration.Zero, 90.minutes).subscribe { _ =>
      refreshCredentialsBox()
    }

    val snykSubscription = Observable.interval(initialDelay + 20.minutes, 1.hour).subscribe { _ =>
      refreshSnykBox()
    }

    val gcpSubscription = Observable.interval(initialDelay + 25.minutes, 90.minutes).subscribe { _ =>
